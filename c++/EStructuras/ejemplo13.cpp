...
void LeerCliente(Cliente listado[])
{
...
// Lectura de archivoEscritura en binario
    ...
    while ( 1 )
	 {
        archivoEntrada.read(reinterpret_cast<char*>( &listado[0] ), sizeof(Cliente) );
        if( archivoEntrada.eof() ) break ;
        cout << left << setw( 20 ) << p_data2[0].nombre << setw( 10 ) << p_data2[0].numUnidades << setw( 10 ) 
            << setprecision( 2 ) << right << fixed << showpoint << p_data2[0].precio << setw( 10 ) << p_data2[0].estado << endl;
     }
